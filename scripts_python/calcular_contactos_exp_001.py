...
"""
SCRIPT: CONTACTOS_EXP-001 – Métricas de contacto microglía–dendrita (slice-wise XY)
...
2) Carga un CSV de grupos con al menos columnas: ["animal", "semana"] (puede incluir otras).
3) Procesa cada TIFF: carga máscaras, calcula región de contacto por slice, deriva métricas.
4) Extrae metadata adicional desde el nombre del archivo (animal, hemisferio) y mergea con el CSV.
...
- hemi: se busca "I" o "C" como campo delimitado por guiones (p.ej. "-I-" o "-C-"):
    "I" → "Ipsi"
    "C" → "Contra"
  Si no matchea, hemi queda NaN (no se fuerza a Ipsi en el código).

Luego:
- Se mergea por "animal" contra el CSV de grupos (mínimo: animal, semana).
- Se crea una variable auxiliar "grupo" = "<semana>_<hemi>" para graficado rápido.
...
- PDF: "graficos_resultados_2px.pdf" en la carpeta output, con gráficos de resumen.
...
print("Seleccioná el CSV con info de grupos (animal, semana)…")
group_csv_path = filedialog.askopenfilename(
    title="CSV de grupos (animal, semana)",
    filetypes=[("CSV files", "*.csv")]
)

out_pdf = os.path.join(output_dir, "graficos_contactos_exp_001.pdf")

# Leemos el CSV de grupos
group_df = pd.read_csv(group_csv_path)

# Aseguramos nombres de columnas esperados
expected_cols = {"animal", "semana"}
if not expected_cols.issubset(set(group_df.columns)):
    raise ValueError(
        f"El CSV de grupos debe tener columnas: {expected_cols}, "
...
# -----------------------------------------------------------
# EXTRAER CÓDIGO DE ANIMAL Y HEMISFERIO DESDE EL NOMBRE
# -----------------------------------------------------------
...
df["hemi"] = df["file"].apply(extraer_hemi)   # Ipsi / Contra

# Merge con info de grupos (animal, semana)
group_df["animal"] = group_df["animal"].astype(str)
df = df.merge(group_df, on="animal", how="left")

# Grupo combinado (semana + hemi) para graficar/analizar rápido
df["grupo"] = df["semana"].astype(str) + "_" + df["hemi"].astype(str)

# Categóricas
df["animal"] = df["animal"].astype("category")
df["semana"] = df["semana"].astype("category")
df["hemi"] = df["hemi"].astype("category")
df["grupo"] = df["grupo"].astype("category")

# Guardar
out_csv = os.path.join(output_dir, "metricas_contactos_exp_001.csv")
